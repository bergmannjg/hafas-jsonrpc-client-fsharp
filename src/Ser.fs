...
type MakeHelpers = 
    static member MakeArray<'T> (items: obj seq): 'T array = 
        [ for i in items do yield i :?> 'T ] |> List.toArray
...
let private makeArray(t: Type, items: obj seq) = 
    typeof<MakeHelpers>.GetMethod("MakeArray").MakeGenericMethod([|t|]).Invoke(null, [|items|])
...
let rec private serializer (options: JsonWriteOptions, t: Type): obj -> string = 
    ...
    if custom.IsSome then 
        ...
        fun outer ->
            let fieldStrings = [|for f in serializers do yield f(outer)|] |> Array.filter (String.IsNullOrEmpty >> not)
...
and fieldSerializer (options: JsonWriteOptions, field: PropertyInfo): obj -> string = 
    ...
    fun outer -> 
        ...
        if (options.filterNoneValue && json = "null") then ""
        else sprintf "%s:%s" name json
...
let rec private deserializer<'T> (options: JsonReadOptions, t: Type): JsonValue -> obj = 
    ...
    if custom.IsSome then 
        ...
        fun j -> 
            let array = j.AsArray()
            let parse = Seq.map deserializeInner array
            let list = makeArray(innerType, parse)
            box(list)
    elif isMap t then 
